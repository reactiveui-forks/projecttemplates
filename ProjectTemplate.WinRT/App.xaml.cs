...
namespace ProjectTemplate
{
    ...
    sealed partial class App : Application
    {
        ...
        public App()
        {
            ...
            Locator.CurrentMutable.RegisterConstant(new IntStringConverterBecauseWinRTSucks(), typeof(IBindingTypeConverter));
        }
    ...
    public class IntStringConverterBecauseWinRTSucks : IBindingTypeConverter
    {
        public int GetAffinityForObjects(Type lhs, Type rhs)
        {
            if (lhs == typeof(int) && rhs == typeof(string) ||
                lhs == typeof(string) && rhs == typeof(int)) 
            {
                return 15;
            }
        ...
        {
            if (from is int && toType == typeof(string)) 
            {
                result = from.ToString();
                return true;
            }

            if (from is string && toType == typeof(int))
            {
                var to = default(int);
                bool ret = Int32.TryParse((string)from, out to);
                result = to;
                return ret;
            }
